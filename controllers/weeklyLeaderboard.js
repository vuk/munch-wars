...
playFabServer.GetLeaderboard({
  ...
  while (newWinners.length < 5 && i < res.data.Leaderboard.length) {
    try {
      ...
      if (newWinners.length < 1) {
        ...
        // Send mail to first 5 potential winners
        sendWinner({
          ...
          '\n' +
          'munchwars.com',
          html: '<p>Kao najbolje plasirani igrač u prethodnoj nedelji, osvojili ste veliku MunchWars kutiju!</p>' +
  ...
  sendWinner({
    ...
    subject: 'Dobitnik nedeljne nagrade ' + date.getDate() + '.' + (date.getMonth() + 1) + '.' + date.getFullYear(),
    attachments: [
      {   // utf-8 string as an attachment
        filename: 'Dobitnici nedeljne nagrade ' + date.getDate() + '.' + (date.getMonth() + 1) + '.' + date.getFullYear() + '.json',
